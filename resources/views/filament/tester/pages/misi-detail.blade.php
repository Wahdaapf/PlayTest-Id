...
<style>
/* ══════════════════ FONTS ══════════════════ */
.md-page, .md-page * { font-family: 'Inter', sans-serif; }
...
@keyframes md-aurora {
    ...
    50%       { transform: translate(8%, -6%)  rotate(180deg) scale(1.2); }
}
...
@media (max-width: 600px) { .md-stats-grid { grid-template-columns: 1fr 1fr; } }
...
.md-instruksi p { margin-bottom: 0.5rem; }
.md-instruksi ul { list-style: disc; padding-left: 1.25rem; margin-bottom: 0.5rem; }
.md-instruksi ol { list-style: decimal; padding-left: 1.25rem; margin-bottom: 0.5rem; }
.md-instruksi h1, .md-instruksi h2, .md-instruksi h3 {
...
.md-btn-primary {
    ...
    padding: 10px 20px; border-radius: 0.875rem;
...
.md-btn-primary:hover { background: linear-gradient(135deg,#1d4ed8,#1e40af); transform: translateY(-1px); box-shadow: 0 6px 20px rgba(37,99,235,0.4); }
...
.md-btn-outline:hover { background: #eff6ff; border-color: #93c5fd; }

.md-btn-apply {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 12px 28px; border-radius: 0.875rem;
    font-size: 0.9rem; font-weight: 700; color: #fff;
    background: linear-gradient(135deg, #2563eb, #7c3aed);
    box-shadow: 0 6px 20px rgba(37,99,235,0.35);
    transition: all 0.2s ease; text-decoration: none; border: none; cursor: pointer;
}
.md-btn-apply:hover { transform: translateY(-2px); box-shadow: 0 10px 28px rgba(37,99,235,0.45); }
...
.md-back-btn {
    display: inline-flex; align-items: center; gap: 6px;
    padding: 8px 14px; border-radius: 0.75rem;
    font-size: 0.8125rem; font-weight: 600; color: #64748b;
    background: #f8fafc; border: 1px solid #e2e8f0;
    text-decoration: none; transition: all 0.15s ease; margin-bottom: 1.25rem;
}
.md-back-btn:hover { background: #f1f5f9; color: #334155; }
...
.md-day-box.done { background: #dbeafe; color: #2563eb; }
.md-day-box.today { background: #2563eb; color: #fff; }

@media (prefers-reduced-motion: reduce) {
    .md-hero, .md-hero-aurora { animation: none !important; }
}
</style>
@endpush
...
<div class="md-page">
    ...
    <div class="md-hero">
        ...
        <div class="relative z-10">
            ...
            <div class="flex items-start gap-4 mb-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        <span class="md-badge" style="background:{{ $cat['bg'] }}22;color:#e0f2fe;border:1px solid rgba(255,255,255,0.2);">
                            {{ $cat['label'] }}
                        </span>
                        @if($misi->paket)
                            <span class="md-badge" style="background:rgba(251,191,36,0.2);color:#fef3c7;border:1px solid rgba(251,191,36,0.4);">
                                ⭐ {{ $misi->paket->name }}
                            </span>
                        @endif
                        @if($isTrusted)
                            <span class="md-badge" style="background:rgba(139,92,246,0.25);color:#ddd6fe;border:1px solid rgba(139,92,246,0.4);">
                                🔒 Trusted Badge
                            </span>
                        @endif
                    </div>
            ...
            <div class="flex items-center gap-3">
                ...
                @endphp
                <span class="md-badge" style="background:{{ $statusColor['bg'] }};color:{{ $statusColor['text'] }};border:1px solid {{ $statusColor['border'] }};">
                    <span class="w-1.5 h-1.5 rounded-full inline-block flex-shrink-0" style="background:{{ $statusColor['dot'] }};"></span>
                    {{ $statusColor['label'] }}
                </span>
                @if($alreadyJoined)
                    <span class="md-badge" style="background:rgba(16,185,129,0.2);color:#a7f3d0;border:1px solid rgba(16,185,129,0.4);">
                        ✓ Sudah Bergabung
                    </span>
                @endif
            </div>
    ...
    <div class="md-stats-grid">
        ...
        <div class="md-stat-card">
            ...
            <div class="md-stat-value">{{ $misi->kapasitas }}<span class="text-base text-slate-400">/{{ $maxCapacity }}</span></div>
    ...
    <div class="md-card">
        ...
        <div class="flex items-center justify-between text-sm mb-2">
            <span class="font-semibold" style="color:#334155;">
                <span class="font-mono-num font-bold" style="color:#1e293b;">{{ $misi->kapasitas }}</span> dari <span class="font-mono-num font-bold" style="color:#1e293b;">{{ $maxCapacity }}</span> tester
            </span>
            <span class="font-mono-num font-bold text-xs" style="color:{{ $kapasitasPersen >= 80 ? '#ef4444' : '#2563eb' }};">{{ $kapasitasPersen }}%</span>
        </div>
        <div class="md-progress-track">
            <div class="md-progress-fill" id="kapasitas-bar" data-target="{{ $kapasitasPersen }}%" style="width:0%;background:linear-gradient(90deg,{{ $kapasitasPersen >= 80 ? '#ef4444,#f97316' : '#2563eb,#7c3aed' }});"></div>
        </div>
        @if($isFull)
            <p class="text-xs mt-2" style="color:#ef4444;font-weight:600;">⚠ Kapasitas penuh — misi ini tidak menerima tester baru.</p>
        @else
    ...
    <div class="md-card">
        ...
        <div class="flex items-center gap-3 mb-3">
            ...
            @endif
            @if($isTrusted)
                <span class="text-xs font-semibold px-3 py-1.5 rounded-full" style="background:#f5f3ff;color:#7c3aed;border:1px solid #ddd6fe;">
    ...
    <div class="mt-2">
            ...
            <div class="flex items-start gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-blue-700 mt-1">
                        Developer telah memulai misi ini. Download aplikasinya, uji sesuai instruksi, lalu kumpulkan laporan harian selama 14 hari.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm mt-1" style="color:#a16207;">
                        @if($maStatus === 'pending')
                            Pendaftaran Anda sedang menunggu persetujuan developer. Anda akan mendapat notifikasi saat diterima.
                        @else
        ...
        <div class="md-card" style="border:none;background:linear-gradient(135deg,#f8fafc,#f1f5f9);text-align:center;padding:2rem;">
            @if($isFull)
                <div class="mb-3" style="background:#fee2e2;border-radius:0.875rem;padding:10px;display:inline-flex;">
                    <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="#ef4444" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M5.07 19H19a2 2 0 001.75-2.98L13.75 4a2 2 0 00-3.5 0L3.25 16.02A2 2 0 005.07 19z"/>
                    </svg>
                </div>
