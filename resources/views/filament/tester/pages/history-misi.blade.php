...
<div class="hist-page px-6 py-6" x-data="{ filter: 'semua' }">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="hist-stat">
            ...
            <div>
                ...
                <p class="text-xs font-medium" style="color:#64748b;">{{ __('Misi Selesai') }}</p>
            </div>
        </div>
        ...
        <div class="hist-stat">
            ...
            <div>
                ...
                <p class="text-xs font-medium" style="color:#64748b;">{{ __('Sedang Aktif') }}</p>
            </div>
        </div>
        ...
        <div class="hist-stat">
            ...
            <div>
                ...
                <p class="text-xs font-medium" style="color:#64748b;">{{ __('Misi Gagal') }}</p>
            </div>
        </div>
        ...
        <div class="hist-stat">
            ...
            <div>
                ...
                <p class="text-xs font-medium" style="color:#64748b;">{{ __('Total Pernah Ikut') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border dark:border-slate-700 overflow-hidden" style="border-color:#e2e8f0;">
        ...
        <div class="flex items-center justify-between flex-wrap gap-3 px-5 py-4" style="border-bottom:1px solid #f1f5f9;">
            <div>
                <h2 class="font-heading text-sm font-bold" style="color:#1e293b;">{{ __('Riwayat Semua Misi') }}</h2>
                <p class="text-xs mt-0.5" style="color:#94a3b8;">{{ $total }} {{ __('misi pernah diikuti') }}</p>
            </div>
            {{-- Filter tabs --}}
            <div class="flex items-center gap-1.5 flex-wrap">
                <button class="hist-tab-active"
                        :class="filter==='semua' ? 'hist-tab-active' : 'hist-tab-inactive'"
                        @click="filter='semua'">{{ __('Semua') }}</button>
                <button :class="filter==='aktif' ? 'hist-tab-active' : 'hist-tab-inactive'"
                        @click="filter='aktif'">{{ __('Aktif') }}</button>
                <button :class="filter==='selesai' ? 'hist-tab-active' : 'hist-tab-inactive'"
                        @click="filter='selesai'">{{ __('Selesai') }}</button>
                <button :class="filter==='gagal' ? 'hist-tab-active' : 'hist-tab-inactive'"
                        @click="filter='gagal'">{{ __('Gagal') }}</button>
            </div>
        </div>
        ...
        @if(count($history) === 0)
            <div class="hist-empty">
                ...
                <p class="text-sm font-medium">{{ __('Belum ada misi yang diikuti') }}</p>
                <p class="text-xs mt-1">{{ __('Temukan misi di halaman dashboard dan mulai pengujian pertamamu!') }}</p>
            </div>
        @else
                    ...
                    <div class="hist-card"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                                ...
                                <span class="text-xs" style="color:#94a3b8;">{{ __('Bergabung') }} {{ $item['joinedAt'] }}</span>
                            </div>
                        </div>
                        ...
                        <div class="text-right flex-shrink-0 hidden sm:block">
                            ...
                            <p class="text-xs" style="color:#94a3b8;">{{ __('Hadiah') }}</p>
                        </div>
            ...
            <div class="hist-empty" x-show="filter !== 'semua'" style="display:none;"
                 x-cloak>
                <p class="text-sm font-medium">{{ __('Tidak ada misi dengan filter ini') }}</p>
            </div>
        @endif
    </div>
    ...
    <div class="mt-4 flex items-start gap-3 px-4 py-3 rounded-xl"
        ...
        <p class="text-xs leading-relaxed" style="color:#b91c1c;">
            <strong>{{ __('Mengapa misi berstatus Gagal?') }}</strong>
            {{ __('Misi dinyatakan gagal jika kamu tidak mengumpulkan laporan harian selama satu hari penuh.') }}
            {{ __('Pastikan submit screenshot bukti tugas setiap hari agar tidak melewatkan tenggat waktu.') }}
        </p>
    </div>
    @endif
