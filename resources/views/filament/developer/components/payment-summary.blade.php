    ...
    <div class="p-5 space-y-6">
        ...
        @if ($paket)
            <div class="rounded-xl bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/10 border border-red-100 dark:border-red-900/50 p-4 relative overflow-hidden">
                ...
                <p class="text-xs font-bold uppercase tracking-widest text-red-600 dark:text-red-400 mb-1">{{ __('Paket Terpilih') }}</p>
        ...
        @else
            <div class="rounded-xl border-2 border-dashed border-slate-200 dark:border-gray-700 p-6 text-center bg-slate-50 dark:bg-gray-800">
                ...
                <p class="text-xs text-slate-500 dark:text-gray-400 font-bold uppercase tracking-tight">{{ __('Belum ada paket dipilih') }}</p>
            </div>
        @endif
        ...
        <div class="space-y-4">
            <div class="flex justify-between items-center text-sm">
                <span class="font-medium text-slate-500 dark:text-gray-400">{{ __('Harga Paket') }}</span>
                <span class="font-bold text-slate-900 dark:text-white">Rp {{ number_format($price, 0, ',', '.') }}</span>
            </div>

            <div class="flex justify-between items-center text-sm">
                <span class="font-medium text-slate-500 dark:text-gray-400">{{ __('Service Fee') }}</span>
            ...
            <div class="border-t-2 border-dashed border-slate-200 dark:border-gray-700 pt-4 flex justify-between items-end mt-2">
                <div>
                    <span class="block text-[10px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-0.5">{{ __('Total Transfer') }}</span>
        ...
        <div class="space-y-3 pt-2">
            <div class="flex gap-3 p-3 rounded-xl bg-amber-50/50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20">
                ...
                <p class="text-[11px] text-amber-800 dark:text-amber-200 leading-relaxed font-medium">
                    {{ __('Misi aktif setelah sistem memverifikasi pembayaranmu. Maksimal') }} <span class="font-bold text-amber-900 dark:text-amber-100">1×24 jam</span>.
                </p>
            </div>
            ...
            <div class="flex items-center gap-2 justify-center py-2 bg-slate-50 dark:bg-gray-800 rounded-lg">
                ...
                <span class="text-[10px] text-slate-500 dark:text-gray-400 font-bold uppercase tracking-widest">{{ __('Pembayaran 100% Aman') }}</span>
            </div>
        </div>
    </div>
</div>