<x-filament-panels::page>
  ...
  <div class="space-y-7 relative">
    ...
    <div>
      <h2 class="text-slate-800 font-bold text-lg mb-1">{{ __('Pilih Kampanye') }}</h2>
      <p class="text-slate-500 text-sm mb-6">{{ __('Pilih aplikasi yang sedang "berjalan" untuk memantau progres testernya.') }}</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between transition-all hover:shadow-md hover:border-blue-300">
          <div>
            ...
            <p class="text-slate-500 text-sm mb-6 line-clamp-2">{{ $m->deskripsi ?? __('Tidak ada deskripsi.') }}</p>
          </div>
        ...
        <div class="col-span-full bg-white border border-slate-200 rounded-2xl p-10 text-center shadow-sm">
          ...
          <h3 class="text-slate-700 font-bold mb-1">{{ __('Belum Ada Kampanye Berjalan') }}</h3>
          <p class="text-slate-500 text-sm">{{ __('Anda belum memiliki aplikasi dengan status \'berjalan\'.') }}</p>
        </div>
        @endforelse
      </div>
    </div>
    ...
    <div class="mb-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
        ...
        <div class="flex items-center gap-2">
          <button wire:click="rejectAllPending" class="text-sm font-semibold px-4 py-2 bg-red-50 text-red-600 border border-red-200 rounded-xl hover:bg-red-100 transition-colors">{{ __('Tolak Semua') }}</button>
        ...
        <div class="bg-white border border-amber-200 rounded-2xl overflow-hidden shadow-sm flex flex-col hover:shadow-md transition-shadow">
          <div class="p-4 bg-amber-50/50 border-b border-amber-100 flex justify-between items-center">
            <div>
              ...
              <p class="text-xs text-slate-500 font-medium">{{ __('Hari ke-:aktif', ['aktif' => $p['hari_ke']]) }} &bull; {{ $p['waktu'] }}</p>
            </div>
            <span class="bg-amber-100 text-amber-700 text-[10px] font-bold px-2 py-1 rounded-md">{{ __('BARU') }}</span>
          </div>
          ...
          <div class="p-4 flex gap-3">
            <button wire:click="rejectDirect({{ $p['id'] }})" class="flex-1 py-2 text-sm font-bold text-red-600 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 hover:border-red-300 transition-colors shadow-sm">{{ __('Tolak') }}</button>
      ...
      <div class="dev-panel-header flex-col sm:flex-row items-start sm:items-center">
        <div class="mb-4 sm:mb-0 w-full sm:w-auto">
          <div class="flex items-center gap-3 mb-1">
            ...
            <h2 class="text-slate-800 font-bold text-base">{{ __('Progres Tester:') }} {{ $misiDetail->nama_aplikasi }}</h2>
          </div>
          <p class="text-slate-500 text-xs mt-0.5 ml-9">{{ __('Klik kotak berwarna pada baris tester untuk meninjau screenshot.') }}</p>
        </div>
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
          <div class="flex flex-wrap items-center gap-3">
            <span class="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap">
              <span class="inline-block w-2.5 h-2.5 rounded-sm" style="background:#fef08a;"></span>{{ __('Perlu Validasi') }}
            </span>
            <span class="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap">
              <span class="inline-block w-2.5 h-2.5 rounded-sm" style="background:#dbeafe;"></span>{{ __('Disetujui') }}
            </span>
            <span class="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap">
              <span class="inline-block w-2.5 h-2.5 rounded-sm" style="background:#fecaca;"></span>{{ __('Ditolak') }}
            </span>
            <span class="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap">
              <span class="inline-block w-2.5 h-2.5 rounded-sm" style="background:#2563eb;"></span>{{ __('Hari Ini') }}
            </span>
            <span class="flex items-center gap-1.5 text-xs text-slate-500 whitespace-nowrap">
              <span class="inline-block w-2.5 h-2.5 rounded-sm" style="background:#fee2e2;"></span>{{ __('Terlewat') }}
            </span>
          </div>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 backdrop-blur-sm" style="background: rgba(15,23,42,0.6);">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col transform transition-all">
      ...
      <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
        <div>
          <h3 class="text-lg font-bold text-slate-800">{{ __('Validasi Screenshot') }}</h3>
          <p class="text-sm text-slate-500 font-medium mt-0.5">{{ $selectedSubData['tester_nama'] }} &bull; {{ __('Hari ke-:aktif', ['aktif' => $selectedSubData['hari_ke']]) }}</p>
        </div>
