...
<style>
    ...
    /* ================================================================
       FLASH NOTIFICATION TOAST — Fixed top-right
       ================================================================ */
    ...
    .login-flash-toast {
        ...
        font-family: 'Manrope', sans-serif;
        line-height: 1.5;
        animation: loginToastIn 0.4s cubic-bezier(0.22, 1, 0.36, 1) both;
        position: relative;
        overflow: hidden;
        box-shadow: 0 10px 30px -5px rgba(0, 0, 0, 0.15), 0 4px 10px -4px rgba(0, 0, 0, 0.1);
    }

    .login-flash-toast::before {
        content: '';
        position: absolute;
        left: 0;
        top: 0;
    ...
    .login-flash-toast.toast-error {
        background: #ffffff;
        border: 1px solid #fecaca;
        color: #991b1b;
    }

    .login-flash-toast.toast-error::before {
        background: #ef4444;
    }

    .login-flash-toast.toast-error .toast-icon {
        color: #ef4444;
    }

    .login-flash-toast.toast-success {
        background: #ffffff;
        border: 1px solid #bbf7d0;
        color: #166534;
    }

    .login-flash-toast.toast-success::before {
        background: #22c55e;
    }

    .login-flash-toast.toast-success .toast-icon {
        color: #22c55e;
    }
...
<x-filament-panels::page.simple>
            ...
            <div class="login-card">
                <div class="login-card-inner">
                    ...
                    <div style="margin-top: 1.75rem;">
                        <a href="{{ route('auth.google', ['panel' => filament()->getCurrentPanel()->getId()]) }}"
    ...
    <div class="login-toast-container" id="login-toast-container">
        @if (session('error'))
        <div class="login-flash-toast toast-error" id="login-flash-toast">
            <span class="toast-icon material-symbols-outlined">error</span>
            <span class="toast-message">{{ session('error') }}</span>
            <button class="toast-close" onclick="this.closest('.login-toast-container').style.animation='loginToastOut 0.3s ease forwards';setTimeout(()=>this.closest('.login-toast-container').remove(),300)" aria-label="Close">&times;</button>
        </div>
        @endif
        @if (session('success'))
        <div class="login-flash-toast toast-success" id="login-flash-toast">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const toast = document.getElementById('login-flash-toast');
        if (toast) {
            setTimeout(function () {
                toast.style.animation = 'loginToastOut 0.3s ease forwards';
